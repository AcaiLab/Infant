...
class VGG16_DANN(nn.Module):
    def __init__(self, num_classes: int):
        ...
        # Emotion classifier head, shared between adult and infant
        self.class_classifier = nn.Linear(feat_dim, num_classes)

        # Domain classifier head, 0 = adult, 1 = infant
    ...
    def forward(self, x, alpha=0.0):
        ...
        # gradient reversal for domain branch
...
def main():
    ...
    # DANN model
    model = VGG16_DANN(num_classes=NUM_CLASSES).to(device)
    print(model)
    ...
    def train_da(model, adult_loader, infant_loader, val_loader,
        ...
        for epoch in range(num_epochs):
            ...
            print(f"Train Total Loss: {epoch_loss:.4f} "
                  f"| Class Loss: {epoch_class_loss:.4f} "
                  f"| Domain Loss: {epoch_domain_loss:.4f} "
                  f"| Class Acc (adult+infant): {epoch_acc:.4f}")

            val_acc = evaluate_classification(model, val_loader, device, desc="Infant val")
    ...
    def evaluate_classification(model, dataloader, device, desc="Eval"):
        model.eval()
        running_corrects = 0
        total = 0
    ...
    test_acc = evaluate_classification(model, infant_test_loader, device, desc="Infant test")
